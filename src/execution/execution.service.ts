...
export class ExecutionService {
  ...
  async executeBatch(
    ...
    try {
      ...
      if (langConfig.compile) {
        const compileResult = await this.compileSource(
          boxId,
          sourceFile,
          langConfig,
          boxPath,
      ...
      return this.buildErrorResults(
        testCases,
        `Internal execution error, ${error}`,
        'SE',
      );
    } finally {
  ...
  private async initBox(boxId: number): Promise<string> {
    const { stdout } = await execAsync(`isolate --box-id=${boxId} --init`);
  ...
  private async compileSource(
    ...
    try {
      ...
      try {
        compileError = await readFile(
          join(boxPath, 'box', 'compile_error.txt'),
          'utf-8',
        );
      } catch {
  ...
  private async runTestCase(
    ...
    try {
      ...
      const inputRedirect = testCase.input ? '< input.txt' : '< /dev/null';
      ...
      try {
        ...
        this.logger.warn(
          `Execution command failed for box ${boxId}, test case ${testCase.name}`,
        );
      }
      ...
      if (!meta) {
        return this.buildResult({
          error: 'Failed to read execution metadata',
          status: 'SE',
        });
      }
      ...
      await execAsync(`rm -f ${metaFile}`).catch(() => {});
    }
  }

  private async readMeta(metaFile: string): Promise<IsolateMeta | null> {
    try {
      const { stdout } = await execAsync(`cat ${metaFile}`);
      return this.parseMetaFile(stdout);
    } catch (error) {
  ...
  private parseMetaFile(content: string): IsolateMeta {
    ...
    for (const line of content.split('\n')) {
      ...
      if (colonIdx > 0) {
        raw[line.substring(0, colonIdx).trim()] = line
  ...
  private async readBoxFile(
    boxPath: string,
    filename: string,
  ): Promise<string> {
    try {
      const { stdout } = await execAsync(`cat ${boxPath}/box/${filename}`);
      return stdout;
    } catch {
      return '';
    }
